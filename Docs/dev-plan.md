...
- [!] Удалить/изолировать нецелевые зависимости, не относящиеся к receiver-domain (в `Api` зарегистрированы `TestService`/`SMSGatewayService`, требуется решение по их месту в receiver-only).
- [ ] Подготовить технический ADR по выбранной структуре (оставляем `Api` как host receiver или создаём `Receiver.Worker`).

### 2.2 Конфигурации и environment
- [ ] Дополнить `appsettings` параметрами receiver-пайплайна и RabbitMQ.
- [ ] Описать env vars и секреты.
- [!] Добавить валидацию конфигурации на старте (есть базовая проверка `AppConfiguration != null` и `AllowedOrigins`, но нет строгой валидации всех receiver-настроек).

### 2.3 Базовые cross-cutting механизмы
- [!] Структурированные логи (включено `AddLogging`, но формат/обогащение не формализованы).
- [ ] Корреляция по `MessageId`/`TraceId`.
- [!] Единый формат ошибок уровня API/processing (есть middleware обработки исключений, но нет подтверждённого общего контракта).
...
### 6.1 Read endpoints
- [ ] Реализовать endpoints списка/карточки доменов.
- [ ] Реализовать endpoints списка/карточки торгов.
- [ ] Реализовать endpoints истории наблюдений/статусов/цен.
- [ ] Реализовать endpoint финальных результатов.
